...
describe("document link helpers", () => {
  const files: readonly MarkdownFile[] = [
    makeFile("README.md"),
    makeFile("docs/guide.md"),
    makeFile("notes/Daily Note.md"),
  ];
  ...
  it("classifies remote Markdown URLs separately from browser links", () => {
    expect(firstDocumentLink("[pricing](https://telnyx.com/pricing.md)")).toEqual({
      kind: "remoteMarkdown",
      url: "https://telnyx.com/pricing.md",
    });
    expect(firstDocumentLink("https://telnyx.com/pricing.md")).toEqual({
      kind: "remoteMarkdown",
      url: "https://telnyx.com/pricing.md",
    });
    expect(firstDocumentLink("https://telnyx.com/pricing")).toEqual({
      kind: "external",
      url: "https://telnyx.com/pricing",
    });
  });
  ...
  it("finds links on rendered rows without raw source-line indexing", () => {
    ...
    expect(firstRenderedDocumentLink("Pricing (https://telnyx.com/pricing.md)")).toEqual({
      kind: "remoteMarkdown",
      url: "https://telnyx.com/pricing.md",
    });
    expect(firstRenderedDocumentLink("Site (https://example.com)")).toEqual({
      kind: "external",
  ...
  it("returns the earliest valid rendered link candidate", () => {
    expect(firstRenderedDocumentLink("Guide (guide) and Site (https://example.com)")).toEqual({
      kind: "internal",
      target: "guide",
    });
    expect(firstRenderedDocumentLink("Guide (docs/guide.md) and Pricing (https://telnyx.com/pricing.md)")).toEqual({
      kind: "internal",
  ...
  it("finds the rendered link under the cursor instead of the first link on the row", () => {
    ...
    expect(renderedDocumentLinkAt(line, line.indexOf("pricing.md") + 1)).toEqual({
      kind: "remoteMarkdown",
      url: "https://telnyx.com/pricing.md",
    });
  ...
  it("finds bare rendered URLs under the cursor in command snippets", () => {
    ...
    expect(renderedDocumentLinkAt(line, line.indexOf("telnyx") + 1)).toEqual({
      kind: "remoteMarkdown",
      url: "https://telnyx.com/pricing.md",
    });
  });
  ...
  it("resolves relative markdown links and wikilinks to discovered files", () => {
    expect(resolveInternalMarkdownFile(files, makeFile("README.md"), "docs/guide.md")?.relativePath).toBe(
      "docs/guide.md",
    );
    expect(resolveInternalMarkdownFile(files, makeFile("docs/current.md"), "guide")?.relativePath).toBe(
      "docs/guide.md",
    );
    expect(resolveInternalMarkdownFile(files, makeFile("README.md"), "notes/Daily Note")?.relativePath).toBe(
      "notes/Daily Note.md",
    );
  });
});
