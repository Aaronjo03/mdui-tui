...
MDUI is a Bun-powered terminal Markdown renderer built with strict TypeScript and OpenTUI. It opens a fast Markdown finder for a directory, renders documents in a read-only TUI, and adds vim-style navigation, visual selection, Slack/PDF export, line numbers, clickable links, and a compact in-app help panel.
...
- Read-only vim-style navigation with counts (`8j`, `4k`), `gg`/`G`, visual and visual-block selection.
- Visual-only line numbers that are excluded from copied content.
- Clipboard helpers: copy selection/document, copy Slack mrkdwn, and export PDF to `~/Downloads`.
...
| `o` or `Enter` | Open the first link on the current line in the browser |
...
MDUI remembers the cursor line and scroll position per file for the current session only. Line numbers are visual-only and are not part of copied Markdown content.
